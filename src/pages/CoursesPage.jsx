...
const CoursesPage = () => {
  ...
  return (
    <motion.div className="p-6 max-w-6xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <h2 className="text-4xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-violet-500 text-center">
        Manage Courses
      </h2>
      ...
      <motion.form
        ...
        className="space-y-4 mb-12 bg-gradient-to-br from-purple-50 via-violet-50 to-purple-100 dark:from-slate-800 dark:to-slate-700 p-6 rounded-xl shadow-lg border border-purple-200 dark:border-slate-700"
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.4 }}
      >
        <h3 className="text-xl font-semibold mb-2 text-purple-600 dark:text-violet-300">Create New Course</h3>
        ...
        <input type="text" placeholder="Media URL" value={form.mediaUrl} onChange={(e) => setForm({ ...form, mediaUrl: e.target.value })}
          className="w-full px-4 py-2 rounded border border-purple-200 dark:border-slate-600 dark:bg-slate-700 text-slate-900 dark:text-white" />
        <button type="submit" className="bg-gradient-to-r from-purple-600 to-violet-500 hover:from-purple-700 hover:to-violet-600 text-white px-5 py-2.5 rounded-md font-semibold transition shadow-md">
          ➕ Create Course
        </button>
      </motion.form>
      ...
      {courses.length === 0 ? (
        ...
        <motion.div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
          {courses.map((course) => (
            <motion.div key={course.courseId} className="bg-white dark:bg-slate-800 p-5 border border-purple-200 dark:border-slate-700 rounded-2xl shadow-lg hover:shadow-xl transition-all" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
              <h4 className="text-lg font-semibold text-purple-600 dark:text-violet-300">{course.title}</h4>
              <p className="text-sm text-slate-600 dark:text-slate-300 mb-2">{course.description || 'No description'}</p>
              {course.mediaUrl && (
                <a href={course.mediaUrl} className="text-sm text-violet-500 hover:underline transition" target="_blank" rel="noopener noreferrer">📺 View Media</a>
              )}
